...
3. Each exercise is designed to be able to be completed in 1-2 hours.
...
(for example, same user in two different groups).
...
3. Add functionality in the Bank and class to do the following
...
### What is provided
1. Classes for the GE90, the GENx, and the GEPassport
...
1. Update as necessary to accomplish the following
    ...
    - Add a new class for the upcoming GE9x engine
...
1. Once the sell by date has passed, quality goes down at twice the normal rate
2. The value of an item can never go below 0
3. Ageable items improve in quality over time. Each day their quality goes up by one. The rate doubles if past sellBy
4. The value of an item never goes above 50
5. Precious items never decrease in quality and never need to be sold
6. Rare items increase in value over time like ageable items, however, within 14 days of the sell by the quality improves by 2x the normal rate,
and within 7 days the quality improves by 3x the normal rate. After the sellBy the quality drops to 0.
...
1. Update the software as necessary to accomplish the following
    ...
    - Perishable items degarde in quality twice as fast as normal items
